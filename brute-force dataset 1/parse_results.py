...
def find_k_best(fname, k=20):
    ...
    for i in range(min(k, len(res_avg))):
        arr.append([
            ...
            res_nb[i][0], ', '.join([x.strip() for x in features[res_nb[i][1]]]),
            res_knn[i][0], ', '.join([x.strip() for x in features[res_knn[i][1]]]),
            res_lr[i][0], ', '.join([x.strip() for x in features[res_lr[i][1]]])
        ])
